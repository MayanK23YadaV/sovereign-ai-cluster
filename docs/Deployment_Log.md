...
## 3. What We Learned
*   **Dynamic IP Shifting (DHCP):** We learned that because this is a university lab environment without static IP assignments, worker nodes will frequently change their IP addresses when they reboot or are powered off for long periods. If nodes suddenly appear "UNREACHABLE" in Ansible, it means their IPs have changed. The solution is to simply re-run `discover_nodes.sh`, which instantly scans the network, finds their new IPs, and rewrites the `/etc/ansible/hosts.ini` inventory automatically.
